...
export default function CollaborationsCMS() {
  ...
  const openModal = (item = null) => {
    ...
    if (item) {
      setFormData({
        ...
        category: item.category || "Commercial Campaign",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isNew
        ? "/api/v1/cms/collaborations"
        : `/api/v1/cms/collaborations/${editingItem._id}`;
      ...
      setSavedMsg(`Brand collaboration ${isNew ? "created" : "updated"} successfully!`);
      setTimeout(() => setSavedMsg(""), 3000);
      setIsModalOpen(false);
      fetchCollaborations();
    } catch (err) {
      console.error("Submit error:", err);
      setError(err.message || "Failed to save brand collaboration.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleStatus = async (item) => {
    ...
    setError(null);
    try {
      await apiFetch(`/api/v1/cms/collaborations/${item._id}/status`, {
        method: "PATCH",
      ...
      setError(err.message || "Failed to update brand collaboration status.");
    }
  };

  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this brand collaboration?")) return;
    setError(null);
    try {
      await apiFetch(`/api/v1/cms/collaborations/${id}`, {
        method: "DELETE"
      });
      setSavedMsg("Brand collaboration deleted successfully.");
      setTimeout(() => setSavedMsg(""), 3000);
      fetchCollaborations();
    } catch (err) {
      console.error("Delete error:", err);
      setError(err.message || "Failed to delete brand collaboration.");
    }
  };

  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) return;
    if (!confirm(`Delete ${selectedIds.length} selected brand collaborations?`)) return;

    setError(null);
    try {
      await apiFetch("/api/v1/cms/collaborations/bulk-delete", {
        method: "POST",
      ...
      setError(err.message || "Failed to delete selected brand collaborations.");
    }
  };
  ...
  const {
    ...
    items,
    search,
    (item) => item.title || item.brandName,
    (item) => item.active
  );
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-sage-border pb-4">
        <div>
          ...
          <h1 className="font-heading text-3xl text-forest">Brand Collaborations</h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="h-4 w-4" /> Add Brand Campaign
          </button>
        </div>
      </div>
      ...
      <div className="relative max-w-md w-full">
        ...
        <input
          type="text"
          placeholder="Search brand collaborations..."
          value={search}
            ...
            >
              <div className="h-40 w-full rounded-2xl overflow-hidden bg-forest/5 border border-sage-border relative">
                {item.heroImage || item.coverImage ? (
                  <img src={mediaUrl(item.heroImage || item.coverImage)} alt="" className="h-full w-full object-cover" />
                ) : (
                  <div className="h-full w-full flex items-center justify-center text-xs text-sage-muted font-semibold">
                    No Campaign Hero Image
                  </div>
                )}
                <input
                  type="checkbox"
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-sage-muted">{item.brandName || item.brand}</p>
                </div>
                ...
                <div className="flex gap-1 shrink-0">
                  <CmsOrderControls
                    ...
                    itemId={item._id}
                    endpoint="/api/v1/cms/collaborations"
                    onError={setError}
                  />
                  <button
                    type="button"
          ...
          <div className="relative max-w-xl w-full max-h-[92vh] rounded-3xl border border-sage-light/40 bg-sage-card shadow-deep flex flex-col overflow-hidden my-auto">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-sage-border/60 bg-sage-card shrink-0">
              <h2 className="font-serif text-2xl text-forest">
                {editingItem ? "Edit Brand Collaboration" : "Add Brand Collaboration"}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0">
              <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 no-scrollbar bg-sage-card text-left">
                <MediaUploadDropzone
                  ...
                  description="Drag and drop campaign hero photo here"
                />
              ...
              <div className="flex items-center justify-end gap-3 p-4 sm:p-5 border-t border-sage-border/60 bg-sage-card/95 backdrop-blur-sm shrink-0">
                ...
                >
                  {saving ? "Saving..." : "Save Brand Collaboration"}
                </button>
              </div>
            </form>
          </div>
