...
export default function WeddingGalleryCMS() {
  ...
  const openModal = (item = null) => {
    ...
    if (item) {
      setFormData({
        title: item.title || "",
        location: item.location || "Château de Chantilly • France",
        coupleName: item.coupleName || "",
        category: item.category || "Luxury Wedding",
        coverImage: item.coverImage || item.thumbnail || "",
        gallery: Array.isArray(item.gallery) ? item.gallery.map((g) => (typeof g === "string" ? g : g.url)) : [],
        storySummary: item.storySummary || item.description || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isNew
        ? "/api/v1/cms/wedding-gallery"
        : `/api/v1/cms/wedding-gallery/${editingItem._id}`;
      ...
      setSavedMsg(`Wedding showcase ${isNew ? "created" : "updated"} successfully!`);
      setTimeout(() => setSavedMsg(""), 3000);
      setIsModalOpen(false);
      fetchGallery();
    } catch (err) {
      console.error("Submit error:", err);
      setError(err.message || "Failed to save wedding showcase.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleStatus = async (item) => {
    ...
    setError(null);
    try {
      await apiFetch(`/api/v1/cms/wedding-gallery/${item._id}/status`, {
        method: "PATCH",
      ...
      setError(err.message || "Failed to update wedding showcase status.");
    }
  };

  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this wedding gallery showcase?")) return;
    setError(null);
    try {
      await apiFetch(`/api/v1/cms/wedding-gallery/${id}`, {
        method: "DELETE"
      });
      setSavedMsg("Wedding showcase deleted successfully.");
      setTimeout(() => setSavedMsg(""), 3000);
      fetchGallery();
    } catch (err) {
      console.error("Delete error:", err);
      setError(err.message || "Failed to delete wedding showcase.");
    }
  };

  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) return;
    if (!confirm(`Delete ${selectedIds.length} selected wedding showcases?`)) return;

    setError(null);
    try {
      await apiFetch("/api/v1/cms/wedding-gallery/bulk-delete", {
        method: "POST",
      ...
      setError(err.message || "Failed to delete selected wedding showcases.");
    }
  };
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-sage-border pb-4">
        <div>
          ...
          <h1 className="font-heading text-3xl text-forest">Wedding Gallery</h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="h-4 w-4" /> Add Wedding Showcase
          </button>
        </div>
      </div>
      ...
      <div className="relative max-w-md w-full">
        ...
        <input
          type="text"
          placeholder="Search wedding gallery..."
          value={search}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-site bg-sage-secondary/70 px-2 py-0.5 rounded-full border border-[rgb(72,125,72)]/20">
                    {item.location || "Wedding Showcase"}
                  </span>
                  <h3 className="font-serif text-xl text-forest font-semibold mt-1">{item.title}</h3>
                  <p className="text-xs text-sage-muted">{item.coupleName}</p>
                </div>
                ...
                <div className="flex gap-1 shrink-0">
                  <CmsOrderControls
                    ...
                    itemId={item._id}
                    endpoint="/api/v1/cms/wedding-gallery"
                    onError={setError}
                  />
                  <button
                    type="button"
          ...
          <div className="relative max-w-xl w-full max-h-[92vh] rounded-3xl border border-sage-light/40 bg-sage-card shadow-deep flex flex-col overflow-hidden my-auto">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-sage-border/60 bg-sage-card shrink-0">
              <h2 className="font-serif text-2xl text-forest">
                {editingItem ? "Edit Wedding Showcase" : "Add Wedding Showcase"}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden min-h-0">
              <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 no-scrollbar bg-sage-card text-left">
                <MediaUploadDropzone
                  ...
                  label="Wedding Cover Image"
                  description="Drag and drop wedding cover photo here"
                />
                ...
                <MediaUploadDropzone
                  ...
                  multiple
                  maxFiles={10}
                  label="Wedding Gallery Photos (Multi-Upload)"
                  description="Drag and drop additional gallery photos here"
                />
              ...
              <div className="flex items-center justify-end gap-3 p-4 sm:p-5 border-t border-sage-border/60 bg-sage-card/95 backdrop-blur-sm shrink-0">
                ...
                >
                  {saving ? "Saving..." : "Save Wedding Showcase"}
                </button>
              </div>
            </form>
          </div>
